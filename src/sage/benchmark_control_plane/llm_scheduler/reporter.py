...
"""
LLM Benchmark Reporter Module
=============================

Generates reports from LLM benchmark results.
...
if TYPE_CHECKING:
    from .runner import LLMBenchmarkResult, LLMPolicyResult


class LLMBenchmarkReporter:
    """Generates LLM benchmark reports in various formats.
    ...
    def __init__(self, result: LLMBenchmarkResult):
        """Initialize reporter.

        Args:
            result: LLMBenchmarkResult to report on
        """
        self.result = result
    ...
    def generate_detailed_report(self) -> str:
        ...
        for policy_name, policy_result in self.result.policy_results.items():
            ...
            if policy_result.gpu_metrics:
                ...
                lines.append(f"    Samples: {gpu.samples}")
                lines.append(f"    Avg Utilization: {gpu.utilization_avg:.1f}%")
                lines.append(f"    Max Utilization: {gpu.utilization_max:.1f}%")
                lines.append(f"    Avg Memory Used: {gpu.memory_used_avg_mb:.0f} MB")
                lines.append(f"    Max Memory Used: {gpu.memory_used_max_mb:.0f} MB")
                lines.append("")
...
def format_policy_result(policy_result: LLMPolicyResult) -> dict[str, Any]:
    """Format a policy result for display.

    Args:
        policy_result: LLMPolicyResult to format
